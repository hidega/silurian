...
function GetFileHandler(context, ioaFactory, params) {
  ...
  self.handle = () => {
    const openFile = (filePath, contentType) => fs.open(filePath, 'r', (err, fd) => err ? error(404) : readFile(fd, contentType))
    try {
      ...
      fs.lstat(filePath, (err, stats) => err || stats.isDirectory() ? error(404) : openFile(filePath, contentType))
    } catch (e) {
      error(500)
    }
  }
